...
/*
Robot app die berichten van andere robots en de controller verwerkt.
...
public class MainActivity extends ServiceActivity {
    ...
    private static final int COMMAND_R = 5;
    ...
    private Button Stop;
    private Button Scan;
    private String HuidigeTikker = "Robot 1";
    private static BluetoothDevice controller;
    private static boolean controllerSet = false;
    private static boolean scanStop = false;
    ...
    private void attachControls() {
        ...
        turnRight = (Button)findViewById(R.id.test1);
        ...
        NorthEast = (Button)findViewById(R.id.bNorthEast);
        ...
        SouthEast = (Button)findViewById(R.id.bSouthEast);
        ...
        SouthWest = (Button)findViewById(R.id.bSouthWest);
        ...
        NorthWest = (Button)findViewById(R.id.bNorthWest);
    ...
    //Functie die wordt aangeroepen nadat er is gescant door de tikker.
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (scanResult != null) {
            String barcode;
            String type;
            String niksGescant = "Niks gescand!!";
            barcode = scanResult.getContents();
            type = scanResult.getFormatName();

            TextView etBarcode = (TextView) findViewById(R.id.textView2);
            etBarcode.setText(barcode);

            //Als er niks is gescant, informeer dan de controller.
            if(barcode == null){
                BluetoothService bluetooth = getBluetooth();
                if (bluetooth != null) {
                        bluetooth.master.sendToDevice(controller,niksGescant);
                        toastShort("niksGescant verstuurd naar de controller\n");
                    }
            }
            //Als er er wel iemand is getikt, informeer dan de buurman en de controller.
            else {
                BluetoothService bluetooth = getBluetooth();
                ...
                if (bluetooth != null) {
                    bluetooth.master.sendToAll(barcode);
                    if(!(HuidigeTikker.equals(barcode))){
                        HuidigeTikker = barcode;
                    }
                }
            }
        }
        else{
            Log.e("hi","niks gescant NULL");
        }
    }


    //Zet het mac-address van de controller.
    private void setController(BluetoothDevice input){
        if(!controllerSet) {
            toastShort("Controller is gezet!");
            controller = input;
            controllerSet = true;
        }
    }
    ...
    //Debug functie voor wanneer de joystick controller niet werkt.
    private void sendRobot(int input){
        BluetoothService bluetooth = getBluetooth();
        if (bluetooth != null) {
            bluetooth.slave.sendToMaster(input);
            toastShort("Succes! verzonden: " + input);
        }
    ...
    private class MainActivityReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // Verwerk data ontvangen als slave.
            if (action.equals(SlaveManager.LISTENER_RECEIVED)) {

                Serializable obj = intent.getSerializableExtra(SlaveManager.EXTRA_OBJECT);
                if (obj != null) {
                    float[] args = getRandomLedArray();
                    String ontvangenData = String.valueOf(obj);

                    //Als de tikker tikt dan moet de robot stilstaan, maar nog wel berichten
                    //kunnen ontvangen.
                    if(ontvangenData.equals("-3")){
                        args[0] = 0;
                        ...
                        if(!(HuidigeTikker.equals(ontvangenData))) {
                            BluetoothService bluetooth = getBluetooth();
                            // Create a handler
                            if (bluetooth != null) {
                                bluetooth.master.sendToAll("-3");
                            }
                        }
                    }

                    //Als er een niewe tikker is, update dan de lokale variabele.
                    else if(!(HuidigeTikker.equals(ontvangenData))) {
                        scanStop = false;
                        HuidigeTikker = ontvangenData;
                        BluetoothService bluetooth = getBluetooth();
                        // Create a handler
                        if (bluetooth != null) {
                            bluetooth.master.sendToAll(ontvangenData);
                        }
                    }
                } else {
            ...
            // Verwerk data ontvangen als master (kunnen alleen berichten van de controller zijn).
                ...
                if (obj != null && !scanStop) {
                    float[] args = getRandomLedArray();
                    String ontvangenData = String.valueOf(obj);

                    args[0] = Integer.parseInt(ontvangenData);

                    //Begin met tikken (start een scan).
                    if (args[0] == -1) {
                        args[0] = 0;
                        getMbed().manager.write(new MbedRequest(COMMAND_R, args));
                        BluetoothService bluetooth = getBluetooth();
                    ...
                    //Dit zijn commandos voor het bewegen van de robot.
                    else {
                        editText.setText(String.valueOf(args[0]));
                        getMbed().manager.write(new MbedRequest(COMMAND_R, args));
                    }
